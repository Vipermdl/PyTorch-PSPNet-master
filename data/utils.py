...
def net_weighing(dataloader, num_classes, cfg, c=1.02):
    ...
    """
    cache_path = osp.abspath(osp.join(cfg.dataset_dir, 'net_weight_cache'))
    if not os.path.exists(cache_path):
        os.makedirs(cache_path)
    cache_file = os.path.join(cache_path, 'class_weights.pkl')
    if os.path.exists(cache_file):
        with open(cache_file, 'rb') as fid:
            class_weights = pickle.load(fid)
    ...
    with open(cache_file, 'wb') as fid:
        pickle.dump(class_weights, fid, pickle.HIGHEST_PROTOCOL)
    print('wrote class_weights to {}'.format(cache_file))
    return class_weights


def median_freq_balancing(dataloader, num_classes, cfg):
    ...
    """
    cache_path = osp.abspath(osp.join(cfg.dataset_dir, 'median_freq_balancing_cache'))
    if not os.path.exists(cache_path):
        os.makedirs(cache_path)
    cache_file = os.path.join(cache_path, 'class_weights.pkl')
    if os.path.exists(cache_file):
...
def class_weighing(label, num_classes, c=1.02):
    """Computes class weights as described in the ENet paper:
    ...
    # Compute propensity score and then the weights for each class
    propensity_score = class_count / flat_label.size
    #class_weights = 1 / (np.log(c + propensity_score))
    class_weights = softmax(propensity_score)
