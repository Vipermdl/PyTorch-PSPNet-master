...
from data.visualize import Visualizer
from args import get_arguments
from data.utils import net_weighing, median_freq_balancing
import utils
...
def load_dataset(dataset):
    ...
    if args.mode.lower() == 'test':
        images, labels, class_labels = iter(test_loader).next()
    else:
        images, labels, class_labels = iter(train_loader).next()
    print("Image size:", images.size())
    print("Label size:", labels.size())
    print("Class label size:", class_labels.size())
    ...
    class_weights = 0
    if args.weighing.lower() == 'net':
        class_weights = net_weighing(train_loader, num_classes, args)
    elif args.weighing.lower() == 'mfb':
        class_weights = median_freq_balancing(train_loader, num_classes, args)
    else:
        class_weights = None
...
if __name__ == '__main__':
    ...
    if args.visdom:
        visdom = Visualizer(env='PSPNet')
    
    if args.mode.lower() in {'train', 'full'}:
        model = train(train_loader, val_loader, w_class, class_encoding, visdom)
